...
def expand_valid_time_coord(data: xarray.DataArray, lead_time: int,
                            interval: int) -> xarray.DataArray:
    ...
    # data
        ...
        if data_interval != interval:
            raise ValueError('\'interval\' is different than temporal '
                             'resolution of data.')
    # Transform time_coord to datetime format
    time_coord = [datetime.utcfromtimestamp(dt.astype(int) / 1e9)
                  for dt in time_coord]
    ...
    if np.mod((end_date_pad - start_date_pad).total_seconds() / 3600,
              interval) == 0:
        num_pad = int(((end_date_pad - start_date_pad).total_seconds()
                      / 3600) / interval) - 1
    else:
        raise ValueError('\'lead_time\' is not a multiple of \'interval\'. '
                         'Padding of xarray is discarded.')

    # Define new valid_time coordinate
    valid_time_coord = pd.date_range(start=time_coord[0], end=end_date_pad,
                                     freq=str(interval) + 'H',
                                     inclusive='left')
